...
void LottoMenu(){
    ...
    std::cout << "[1] - Automatically generate random lotto numbers" << std::endl;
    std::cout << "[2] - Create your own lotto numbers              " << std::endl << std::endl;
    int choice = 0;
            ...
            while (!doneGen){
                ...
                while (choice < 1 || choice > 2){
                    ...
                    if (choice < 1 || choice > 2 || std::cin.fail() || std::cin.peek()!='\n' ){ //std::cin.fail() checks wether the input fits the variable. std::cin.peek will in this case check that the next character in isteam is a "new line", "ENTER" key input basically, and not something else
                        ...
                        doneGen = 1; //If user chooses to end generating their own numbers. doneGen is set to true
                        std::cout << std::endl;
                        break;
                    }
                }
